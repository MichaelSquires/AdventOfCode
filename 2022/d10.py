...
most pixel in each row is in position 0, and the right-most pixel in each row is
in position 39.
...
screen leaves the pixel dark (.). The first few pixels from the larger example
above are drawn as follows: Sprite position:
###.....................................  Start cycle   1: begin executing addx
15 During cycle  1: CRT draws pixel in position 0 Current CRT row: #  During
cycle  2: CRT draws pixel in position 1 Current CRT row: ## End of cycle  2:
finish executing addx 15 (Register X is now 16) Sprite position:
...............###......................  Start cycle   3: begin executing addx
-11 During cycle  3: CRT draws pixel in position 2 Current CRT row: ##.  During
cycle  4: CRT draws pixel in position 3 Current CRT row: ##.. End of cycle  4:
finish executing addx -11 (Register X is now 5) Sprite position:
....###.................................  Start cycle   5: begin executing addx
6 During cycle  5: CRT draws pixel in position 4 Current CRT row: ##..#  During
cycle  6: CRT draws pixel in position 5 Current CRT row: ##..## End of cycle  6:
finish executing addx 6 (Register X is now 11) Sprite position:
...
def parse(data: str):
    ...
    for line in data.splitlines():
        ...
        if inst == ADDX:
            ret.append((ADDX, int(oper), 2))
        elif inst == NOOP:
            ret.append((NOOP, 0, 1))
        else:
            return NotImplementedError(f'LINE: {line}')
...
def part1(data):
    ...
    for _,oper,count in data:
        for _ in range(count):

            if cycle in CYCLES:
                logging.debug('cycle %d, reg_x %d, total %d', cycle, reg_x, total)
                total += reg_x * cycle
...
def part2(data):
    ...
    for _,oper,count in data:
        for _ in range(count):
            ...
            cursor = reg_x + (COLS * ((cycle - 1) // COLS))
            logging.debug('CURSOR %d', cursor)

            if cursor - 1 == cycle - 1 or cursor == cycle - 1 or cursor + 1 == cycle - 1:
                crt[cycle - 1] = '#'

            cycle += 1

        reg_x += oper
        logging.debug('REG_X %d, cycle %d', reg_x, cycle)

    ret = '\n'
    for ii in range(ROWS):
        ret += ''.join(crt[ii*COLS:(ii+1)*COLS])
        ret += '\n'
    return ret
