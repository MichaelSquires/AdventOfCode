...
class Grid(utils.Grid):
    def pprint(self, xoffset=0, yoffset=0):
        ret = []
        replacements = '.#o'
        for yy in range(self.height):  # pylint: disable=invalid-name
            height = self.width * (yy - yoffset)
            ret.append(self._grid[0+xoffset + height:self.width + height])

        printable = ''
        lineno = 0
        for line in ret:
            printable += f'{lineno:03d} '
            printable += ''.join([replacements[k] for k in line])
            printable += '\n'
            lineno += 1
...
def parse(data: str):
    ...
    for line in data.splitlines():
        ...
        for index in range(len(coords) - 1):
            startx, starty = list(map(int, coords[index].split(',')))
            endx, endy = list(map(int, coords[index+1].split(',')))
            ...
            if starty == endy:
                ...
                for ii in range(start, end+1):
                    rocks.append((ii, starty))

    grid = Grid(maxy + 3, maxx + 1)
    for xy in rocks:
        grid[xy] = ROCK
...
def sand(grid):
    ...
    while not abyss:
        ...
        while True:
            ...
            # Nowhere to move, stop here
            grid[curr] = SAND
            if curr == start:
                abyss = True
            break

    return grid.count(SAND)

def part1(grid: Grid):
    return sand(grid)

def part2(grid: Grid):
    # Extend the width of the grid
    grid.extend(grid.height, grid.width*2)

    # Set the cave floor
    for xx in range(grid.width):
        grid[(xx,grid.height-1)] = ROCK
