...
Each monkey has several attributes:
  ...
  - Test shows how the monkey uses your worry level to decide where to throw an
    item next.    If true shows what happens with an item if the Test was true.
...
class Monkey:
    def __init__(self, mid, items, operation, test, if_true, if_false):
        self.mid = int(mid)
        self.items = [int(k) for k in items.split(',')]
        self.test = int(test)
...
def parse(data: str):
    ...
    for match in PATTERN.finditer(data):
        monkey = Monkey(*match.groups())
        ret[monkey.mid] = monkey

    return ret

P1_ROUNDS = 20

def part1(data: dict[int, Monkey]):
    for _ in range(P1_ROUNDS):
        for monkey in data.values():
            for item in monkey.items:
                ...
                if math.remainder(item, monkey.test) == 0:
                    mid = monkey.if_true
                else:
                    mid = monkey.if_false

                data[mid].items.append(item)

            monkey.items = []

    counts = [m.count for m in data.values()]
    counts.sort()
    counts.reverse()
...
def part2(data):
    # Multiply all the tests together to get a giant modulo that we can use to
    # reduce worry
    divider = math.prod([m.test for m in data.values()])
    ...
    for _ in range(P2_ROUNDS):
        for monkey in data.values():
            for item in monkey.items:
                ...
                monkey.count += 1
                item = monkey.operation(item)
                # Reduce worry with calculated modulo
                # I didn't figure this out myself, I looked at the d11
                # megathread to see what other folks did
                item = item % divider
