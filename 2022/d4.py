...
Section = collections.namedtuple('Section', ['start', 'end'])
...
def parse(data: str):
    ...
    for elf1, elf2 in data:
        e1start, e1end = elf1.split('-')
        e2start, e2end = elf2.split('-')
        e1 = Section(int(e1start), int(e1end))
        e2 = Section(int(e2start), int(e2end))
        ret.append((e1, e2))
    return ret
...
def part2(data):
    ret = 0
    for elf1, elf2 in data:
        if elf1.start > elf2.end or elf2.start > elf1.end:
            ret += 1
    return len(data) - ret
