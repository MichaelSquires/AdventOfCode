...
very much like some modern computers:
  ...
  - ls means list. It prints out all of the files and directories immediately
    contained by the current directory:    123 abc means that the current
    directory contains a file named abc with size 123. dir xyz means that the
    current directory contains a directory named xyz.
...
class Directory:
    ...
    def __init__(self, name):
        ...
        self.DIRS[self.name] = self

    def add_file(self, name, size):
        self.files.append((name, size))

    def add_dir(self, name):
        self.dirs[name] = Directory(str(pathlib.Path(self.name) / name))

    def size(self):
        return sum(k[1] for k in self.files) + sum(k.size() for k in self.dirs.values())

    def cd(self, name):
        if name == '..':
            path = str(pathlib.Path(self.name).parent)
        else:
            path = str(pathlib.Path(self.name) / name)
...
def parse(data: str):
    root = Directory('/')
    curr = root
    data = [k.strip() for k in data.split('$') if k]
    for cmd in data:
        if cmd.startswith('cd'):
            ...
            for line in cmd.splitlines()[1:]:
                ...
                if x == 'dir':
                    ...
                    curr.add_file(y, int(x))
        
    return root

def part1(data):
    dirs = []
    for dir in data.DIRS.values():
        if dir.size() < 100000:
            dirs.append(dir)

    return sum(k.size() for k in dirs)

TOTAL = 70000000
NEEDED = 30000000

def part2(data):
    remaining = TOTAL - data.size()

    sizes = [(k.name, k.size()) for k in data.DIRS.values() if k.size() > NEEDED - remaining]
