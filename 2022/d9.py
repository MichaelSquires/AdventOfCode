...
def adjacent(tail, head):
    ...
    return maxx - minx <= 1 and maxy - miny <= 1
...
def move_tail(tail, head):
    ...
    # Simple horizontal or vertical moves
    # ..H..
    # .....
    # H.T.H
    # .....
    ...
    if diffx > diffy and hx > tx and hy < ty:
        # T..
        # ..H
        return tx + 1, ty - 1

    if diffy > diffx and hy < ty and hx > tx:
        # T.
        # ..
        # .H
        return tx + 1, ty - 1
    ...
    raise NotImplementedError(f'HEAD {head}, TAIL {tail}')
...
def part2(data):
    ...
    for direction,distance in data:
        for _ in range(distance):
            ...
            for tt in range(1, len(knots)):
                head = knots[tt - 1]
                tail = knots[tt]

                tail = move_tail(tail, head)

                logging.debug('TAIL[%d]: %s', tt, tail)
                if tt == len(knots) - 1:
                    visited.add(tail)
