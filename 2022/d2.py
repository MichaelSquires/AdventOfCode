...
def parse(data):
    data = data.split('\n')
    data = [k.replace(' ', '') for k in data if k]
    return data
...
P1_MAP = {
    LROCK + RROCK: ROCK + DRAW,
    LROCK + RPAPER: PAPER + WIN,
    LROCK + RSCISSORS: SCISSORS + LOSE,

    LPAPER + RROCK: ROCK + LOSE,
    LPAPER + RPAPER: PAPER + DRAW,
    LPAPER + RSCISSORS: SCISSORS + WIN,

    LSCISSORS + RROCK: ROCK + WIN,
    LSCISSORS + RPAPER: PAPER + LOSE,
    LSCISSORS + RSCISSORS: SCISSORS + DRAW,
}

def part1(data):
    return sum(map(P1_MAP.get, data))

P2_MAP = {
    LROCK + RDRAW: ROCK + DRAW,
    LROCK + RLOSE: SCISSORS + LOSE,
    LROCK + RWIN:  PAPER + WIN,

    LPAPER + RDRAW: PAPER + DRAW,
    LPAPER + RLOSE: ROCK + LOSE,
    LPAPER + RWIN:  SCISSORS + WIN,

    LSCISSORS + RDRAW: SCISSORS + DRAW,
    LSCISSORS + RLOSE: PAPER + LOSE,
    LSCISSORS + RWIN:  ROCK + WIN,
}

def part2(data):
    return sum(map(P2_MAP.get, data))
