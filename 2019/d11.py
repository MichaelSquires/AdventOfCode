...
MOVES = {
    utils.up: [utils.left, utils.right],
    utils.down: [utils.right, utils.left],
    utils.right: [utils.up, utils.down],
    utils.left: [utils.down, utils.up],
}
...
class Robot:
    ...
    def move(self, val):
        if self.painting:
            self.grid[self.xy] = val
        else:
            self.direction = MOVES[self.direction][val]
            self.xy = self.direction(*self.xy)
            
        self.painting = not self.painting
            
    def camera(self):
        return self.grid[self.xy]
...
def part1(data):
    ...
    mach.infunc = robot.camera
    mach.outfunc = robot.move
...
def part2(data):
    ...
    # Get all x and y vals
    xvals = [x for x,y in robot.grid.keys()]
    yvals = [y for x,y in robot.grid.keys()]

    # Find min and max xvals
    min_x = min(xvals)
    max_x = max(xvals)

    # Find min and max yvals
    min_y = min(yvals)
    max_y = max(yvals)

    # Create grid with calculated values
    grid = Grid(max_y - min_y + 1, max_x - min_x + 1)
    
    # Figure out shift to origin
    xdiff = 0 - min_x
    ydiff = 0 - min_y

    for key,val in robot.grid.items():
        x,y = key
