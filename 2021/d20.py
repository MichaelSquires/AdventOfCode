...
def parse(data):
    ...
    algo = algo.replace('.', '0')
    algo = algo.replace('#', '1')
    algo = list(map(int, algo))
    assert len(algo) == 512

    image = image.replace('.', '0')
    image = image.replace('#', '1')
    image = [list(map(int, k)) for k in image.splitlines()]

    grid = Grid.init_with_data(image)

    return algo, grid

def enhance(algo, grid):
    default = algo[0]
    if default:
        default = int(not grid.default)

    out = Grid(grid.height+2, grid.width+2, default=default)

    for xy in grid.foreach(min_x=-1, min_y=-1, max_x=grid.width+1, max_y=grid.height+1):
        x, y = xy
        val = grid.enhance(x, y)

        out[x+1, y+1] = algo[val]
