...
def parse(data):
    match = re.findall(r'Player (\d) starting position: (\d+)', data)
    return {int(k):[int(v), 0] for k,v in match}
...
def part1(data):
    ...
    while True:
        ...
        rolls = (roller.roll(), roller.roll(), roller.roll())
        ...
        data[curr] = [pos, score]
        #logging.info('Player %s rolls %s and moves to space %s for a total score of %s', curr, rolls, pos, score)
    ...
    return roller.rolls * data[curr][1]
