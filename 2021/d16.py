...
remaining type IDs are more interesting:
  ...
  - Packets with type ID 5 are greater than packets - their value is 1 if the
    value of the first sub-packet is greater than the value of the second sub-
    packet; otherwise, their value is 0. These packets always have exactly two
    sub-packets.
  ...
  - Packets with type ID 6 are less than packets - their value is 1 if the value
    ...
    otherwise, their value is 0. These packets always have exactly two sub-
    packets.
...
class Packet:
    ...
    def read(self, size):
        data = self._packet[self.offset:self.offset+size]
        self.offset += size

        val = int(data, 2)
        logging.debug('READ: %s (%s) -> %s', data, size, val)
        return val
    ...
    def parse(self):
        ...
        # Operator packet
            ...
            if lti == 0:
                ...
                sub_start = self.offset
                while self.offset < sub_start + length:

                    logging.debug('LTI0: %s', length)

                    pkt = Packet(self._packet[self.offset:self.offset+length])
                    sub = pkt.parse()

                    self._subs.append(sub)
                    self.offset += pkt.offset

            else:
                length = self.read(11)

                logging.debug('LTI1: %s', length)

                while len(self._subs) < length:

                    pkt = Packet(self._packet[self.offset:])
                    sub = pkt.parse()

                    self._subs.append(sub)
                    self.offset += pkt.offset

            logging.debug('pkt_type: %s %s', pkt_type, len(self._subs))
            match pkt_type:
                case 0:
                    return sum(self._subs)

                case 1:
                    return math.prod(self._subs)

                case 2:
                    return min(self._subs)

                case 3:
                    return max(self._subs)

                case 5:
                    assert len(self._subs) == 2
                    return int(self._subs[0] > self._subs[1])
                ...
                case _:
                    raise Exception(f'Invalid packet type: {pkt_type}')


def parse(data):
    return ''.join([hexmap[k] for k in data.strip()])
