...
class Grid(utils.Grid):
    ...
    def synchronized(self):
        return self._grid == ([0] * len(self._grid))

    def update(self):
        flashes = 0

        while self.flashing():
            # Copy the grid so we're not reading and updating at the same time
            cgrid = copy.copy(self)
            for x, y in self.foreach():
                val = cgrid[x, y]
                if val > 9 and val < 100:
                    cgrid[x, y] += 100
                    flashes += 1
